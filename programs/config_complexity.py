...
while t < tmax:
    ...
    if t >= nextSampleTime:
        ...
        configs, _, _ = sampler.sample(psi, 10000)
        configs = np.array(configs).reshape((-1, L))
        configs = mpi.comm.gather(configs, root=0)
        if mpi.rank == 0:
            configs = np.concatenate(configs, axis=0)
        outp.write_dataset("samples_t=" + str(t), configs, groupname="sampled_configurations")
